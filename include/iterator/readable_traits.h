...
namespace jmx {
    ...
    struct indirectly_readable_traits<Tp>: cond_value_type<typename Tp::value_type> {};

    template <has_member_element_type Tp>
    struct indirectly_readable_traits<Tp>: cond_value_type<typename Tp::element_type> {};
    ...
    // 消除上面萃取成员的歧义
    template <has_member_value_type Tp>
    requires has_member_element_type<Tp> && std::same_as<std::remove_cv_t<typename Tp::element_type>, std::remove_cv_t<typename Tp::value_type>>
    struct indirectly_readable_traits<Tp>: cond_value_type<typename Tp::value_type> {};
